        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            for idx, detection in enumerate(dr_data):
                ...
                if show_animation:
                    ...
                    if ovmax > 0:
                        ...
                        cv2.putText(
                            img_cumulative, class_name, (bbgt[0], bbgt[1] - 5), font, 0.6, light_blue, 1, cv2.LINE_AA)
            ...
            for idx, val in enumerate(tp):
                rec[idx] = float(tp[idx]) / \
                    np.maximum(gt_counter_per_class[class_name], 1)
            ...
            F1 = np.array(rec)*np.array(prec)*2 / np.where((np.array(prec) +
                                                            np.array(rec)) == 0, 1, (np.array(prec)+np.array(rec)))
            ...
            if len(prec) > 0:
                ...
                Recall_text = "{0:.2f}%".format(
                    rec[score_threhold_idx]*100) + " = " + class_name + " Recall "
                Precision_text = "{0:.2f}%".format(
                    prec[score_threhold_idx]*100) + " = " + class_name + " Precision "
            else:
...
def preprocess_gt(gt_path, class_names):
    ...
    for i, image_id in enumerate(image_ids):
        ...
        for line in lines_list:
            ...
            left, top, right, bottom = float(left), float(
                top), float(right), float(bottom)
            if class_name not in class_names:
                continue
            cls_id = class_names.index(class_name) + 1
            bbox = [left, top, right - left, bottom - top, difficult,
