...
class LossHistory():
    ...
    def append_loss(self, epoch, loss, val_loss, ssd_loss, depth_loss, ssd_loss_v, depth_loss_v):
        if not os.path.exists(self.log_dir):
            os.makedirs(self.log_dir)

        self.losses.append(loss)
        self.val_loss.append(val_loss)
        self.ssd.append(ssd_loss)
        self.depth.append(depth_loss)
        self.ssd_v.append(ssd_loss_v)
        self.depth_v.append(depth_loss_v)
        ...
        with open(os.path.join(self.log_dir, "epoch_val_loss.txt"), 'a') as f:
            ...
            f.write("\n")
        with open(os.path.join(self.log_dir, "epoch_ssd_loss.txt"), 'a') as f:
            f.write(str(ssd_loss))
            f.write("\n")
        with open(os.path.join(self.log_dir, "epoch_depth_loss.txt"), 'a') as f:
            f.write(str(depth_loss))
            f.write("\n")
        with open(os.path.join(self.log_dir, "epoch_ssdv_loss.txt"), 'a') as f:
            f.write(str(ssd_loss_v))
            f.write("\n")
        with open(os.path.join(self.log_dir, "epoch_depthv_loss.txt"), 'a') as f:
            f.write(str(depth_loss_v))
            f.write("\n")
...
class EvalCallback():
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        with torch.no_grad():
            ...
            results = self.bbox_util.decode_box(outputs[0], self.anchors, image_shape, self.input_shape, self.letterbox_image,
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            try:
                ...
                temp_map = get_map(self.MINOVERLAP, False,
